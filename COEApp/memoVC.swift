...
class memoVC: UIViewController, UITextFieldDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
           ...
           func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int){
                ...
                if pickerView == self.prgrmPicker {
                   self.prgrmTF.text = self.prgrms?[row].PROGRAM
                    ...
                    guard let gitUrll = URL(string: rsltExamAPI + "\(self.prgrms?[row].PROGRAM ?? " ")") else { loadingView.hide()
                        return
    ...
    @IBAction func submitAct(_ sender: UIButton) {
        if self.prgrmTF.text?.count == 0 {
            ...
            defaults.set("\(self.prgrmTF.text ?? " ")/\(self.examTF.text ?? " ")/\(self.courseTF.text ?? " ")/\(self.subCourseTF.text ?? " ")", forKey: "memoLink")
            
            let vc:memoDtlVC = self.storyboard?.instantiateViewController(withIdentifier: "memoDtlVC") as! memoDtlVC
